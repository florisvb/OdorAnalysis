...
def plot_odor_heading_book(pp, threshold_odor, path, config, dataset, keys=None):
    ...
    for key in keys:
        ...
        for block in odor_blocks:
            ...
            for sac in trajec.saccades:
                if sac[0] > block[0] and sac[0] < block[-1]:
    ...
    fpl.histogram_stack(ax, [heading_after_saccade*180./np.pi], bins=20, bin_width_ratio=0.9, colors=['red'], edgecolor='none', normed=True)
...
def pdf_book(config, dataset, save_figure_path=''):
    ...
    save_figure_path = os.path.join(figure_path, 'odor_traces/')
    pdf_name_with_path = os.path.join(save_figure_path, 'saccade_angle_after_entering_plume.pdf')
